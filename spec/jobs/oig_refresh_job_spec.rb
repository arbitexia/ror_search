...
describe OIGRefreshJob do
  describe 'when fails' do
    it 'should send an email to all admins' do
      allow_any_instance_of(OIGSearcher).to receive(:regenerate_database).and_raise('some fake error')
      ...
      expect(failure_email.subject).to include 'OIG'
